...
const ModalClientCard = ({item, handleCancel}) => {
  const { confirm } = Modal;
  ...
  useEffect(() => {
    ...
    getUbosCtrlPromise(item.rut, "CHIL").then(ubos => {
      if(ubos.success) {
        setUbos(ubos.data)
      }else{
  ...
  const loadFicha = () => {
    getFichaPromise(item.id).then(ficha => {
      setSelectedRisk(ficha.risk)
      setRiskComment(ficha.commentRisk)
  ...
  const propColumns = [
    {
      dataIndex: item.type === 'Person' || (ubos && ubos.ubos && ubos.ubos.length === 0) ? 'participacionDirecta' : 'participacion',
      width: '20%',
      render: (porcent) => {
